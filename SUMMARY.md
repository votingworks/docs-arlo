# Table of contents

* [Risk-Limiting Audits with Arlo](README.md)
* [Audit Types](audit-types.md)
* [User Roles](user-roles.md)

## Jurisdiction Manager

* [Logging into Arlo](jurisdiction-manager/logging-into-arlo.md)
* [Pre-Audit Files](jurisdiction-manager/pre-audit-file-uploads/README.md)
  * [Ballot Manifest](jurisdiction-manager/pre-audit-file-uploads/ballot-manifest.md)
  * [Cast Vote Records (CVRs)](jurisdiction-manager/pre-audit-file-uploads/cast-vote-records-cvrs.md)
  * [Candidate Totals by Batch](jurisdiction-manager/pre-audit-file-uploads/candidate-totals-by-batch.md)
  * [Uploading Files into Arlo](jurisdiction-manager/pre-audit-file-uploads/uploading-files-into-arlo.md)
* [Audit Conduct](jurisdiction-manager/audit-conduct-1/README.md)
  * [Prepare Audit Materials](jurisdiction-manager/audit-conduct-1/audit-conduct.md)
  * [Batch Entry into Arlo](jurisdiction-manager/audit-conduct-1/batch-entry-into-arlo.md)
* [Audit Completion](jurisdiction-manager/audit-completion.md)
* [Supply List](jurisdiction-manager/supply-list.md)

## Audit Board

* [Ballot Retrieval](audit-board/ballot-retrieval.md)
* [Ballot Entry into Arlo](audit-board/ballot-entry-into-arlo-1.md)
* [Batch Retrieval & Tally](audit-board/batch-retrieval.md)
* [Checklists](audit-board/checklists.md)

## Audit Administrator

* [Account Setup](audit-administrator/hybrid-overview.md)
* [Audit Setup](audit-administrator/ballot-manifest/README.md)
  * [Participants](audit-administrator/ballot-manifest/participants.md)
  * [Standardized Contests File](audit-administrator/ballot-manifest/standardized-contests-file.md)
  * [File Upload Progress](audit-administrator/ballot-manifest/file-upload-progress.md)
  * [Target Contests](audit-administrator/ballot-manifest/target-contests.md)
  * [Opportunistic Contests](audit-administrator/ballot-manifest/opportunistic-contests.md)
  * [Audit Settings](audit-administrator/ballot-manifest/audit-settings.md)
  * [Review & Launch](audit-administrator/ballot-manifest/review-and-launch.md)
* [Audit Progress](audit-administrator/audit-progress.md)
* [Audit Completion](audit-administrator/audit-completion.md)

## Resources

* [Audit Report Guide](resources/audit-report-guide.md)
* [Batch Management](resources/batch-management.md)
* [RLA Model Legislation](resources/rla-model-legislation.md)
* [Training Videos](resources/training-videos.md)

## Georgia Full Manual Tally - November 2020 <a href="full-manual-tally" id="full-manual-tally"></a>

* [Space Setup](full-manual-tally/space-setup.md)
* [Supplies Required](full-manual-tally/supplies-required.md)
* [Audit Roles](full-manual-tally/audit-roles.md)
* [Audit Board Guide](full-manual-tally/audit-board-guide.md)
* [Check-In / Check-Out Guide](full-manual-tally/check-in-check-out-guide.md)
* [Vote Review Panel Guide](full-manual-tally/vote-review-panel-guide.md)
* [Monitors & Observers](full-manual-tally/monitors-and-observers.md)
* [Full Manual Tally – Resources](full-manual-tally/full-manual-tally-all-resources.md)
